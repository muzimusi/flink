...
CURR_DIR=`pwd`
if [[ `basename $CURR_DIR` != "releasing" ]] ; then
  echo "You have to call the script from the releasing/ dir"
  exit 1
fi
...
make_binary_release() {
  ...
  $MVN clean package $FLAGS -Prelease -pl flink-dist -am -Dgpg.skip -Dcheckstyle.skip=true -Dmaven.javadoc.skip=true -DskipTests
